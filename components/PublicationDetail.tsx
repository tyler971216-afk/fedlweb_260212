...
const PublicationDetail: React.FC<PublicationDetailProps> = ({ period, onBack }) => {
  ...
  const renderCoverImages = (images: string[]) => {
    if (!images || images.length === 0) return null;
    return (
      <div className="mb-16 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
        {images.map((img, idx) => (
          <div key={idx} className="cover-item group relative aspect-[440/584] rounded-xl overflow-hidden shadow-lg border border-slate-200 bg-white hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
            <img 
              src={img} 
              alt="Journal Cover" 
  ...
  const renderPubList = (items: PublicationItem[], hideButton: boolean = false, hideImage: boolean = false) => (
    <div className="space-y-8">
      {items.map((item) => {
        const finalLink = item.link || `https://scholar.google.com/scholar?q=${encodeURIComponent(item.text)}`;
        
        return (
          <div key={`${item.num}-${item.text.substring(0, 20)}`} className={`pub-item flex flex-col md:flex-row gap-8 items-start ${hideImage ? 'p-6' : 'p-8'} rounded-3xl bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow group`}>
            {!hideImage && (
            ...
            <div className="flex-grow pt-2">
              <div className="flex items-start gap-4 mb-4">
                {hideImage && (
                  <div className="min-w-[2.5rem] h-10 px-2 bg-[#131C31] text-white flex items-center justify-center font-black rounded-lg text-lg shadow-lg flex-shrink-0">
                    {item.num}
                  </div>
                )}
                <p className={`${hideImage ? 'text-lg' : 'text-xl'} text-[#131C31] leading-relaxed font-medium`}>
                  {item.text}
                </p>
              </div>
  ...
  if (period === 'book') {
    return (
      <div className="min-h-screen bg-[#F8FAFC] pb-20 relative">
        ...
        {getCommonHeader("Book Chapters")}
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="pt-8">
            {renderPubList([
              { num: 10, text: 'Ahn J.-H. and Kim J.H, "Micro Light Emitting Diode: Fabrication and Devices", Springer (2022)', img: '/images/bookchap/10.png' },
              { num: 9, text: 'Sharma B., Das T., and Ahn J.-H., "Graphene for Flexible Electronics", a chapter in, "Flexible Carbon-based Electronics", edited by Houston Samori and Palermo, Wiley-VCH (2018)', img: '/images/bookchap/9.png' },
              { num: 8, text: 'Ahn J.-H and Rogers J., "Silicon Nanomembranes: Fundamental Science and Applications", Wiley-VCH (2016)', img: '/images/bookchap/8.png' },
              { num: 7, text: 'Jang H, Lee W, Kim M. and Ahn J.-H., "Inorganic Semiconductor Nanomaterials for Flexible Electronics", a chapter in, "Large Area and Flexible Electronics", edited by Carina Paraiso, Wiley-VCH (2015)', img: '/images/bookchap/7.png' },
              { num: 6, text: 'Yan C. Lee S., 출력 H. and Ahn J.-H., "Graphene for stretchable electronics", a chapter in, " Stretchable Electronics", edited by T. Someya, Wiley-VCH (2012)', img: '/images/bookchap/6.png' },
              { num: 5, text: 'Yan C. Lee Y, Jang H. and Ahn J.-H., "Graphene films for unusual format electronics", a chapter in, " Graphene systhesis and applications ", edited by Choi W and Lee J, CRC press (2011)', img: '/images/bookchap/5.png' },
              { num: 4, text: 'Ahn J.-H., Kim, D. -H., and Rogers J. A., "Micro and Nanostructured Semiconductor Materials for Flexible and Stretchable Electronics", a chapter in, " Comprehensive Semiconductor Science & Technology ", edited by Bhattacharya Pallab, Elsevier (2011)', img: '/images/bookchap/4.png' },
              { num: 3, text: 'Ahn J.-H., Baca A. J., and Rogers J. A.,“ Inorganic Nanomaterials for High Performance Flexible Electronics”, a chapter in, "Application of Physical Methods to Inorganic and Bioinorganic Chemistry", edited by Robert A Scott and Charles M Lukehart, John Wiley (2009).', img: '/images/bookchap/3.png' },
              { num: 2, text: 'Ahn J.-H., Meitl M. A., Baca A. J., Khang D. Y., Kim H. S., and Rogers J. A.,“ Transfer Printing Techniques and Inorganic Single-Crystalline Materials for Flexible and Stretchable Electronics”, a chapter in, "Solution Processing of Inorganic Materials", edited by David Mitzi, John Wiley & Sons (2009).', img: '/images/bookchap/2.png' },
              { num: 1, text: 'Sun Y., Ahn J.-H. Rogers J.A., “Printable semiconductors for flexible electronics”, McGraw-Hill Yearbook of Science & Technology, 192-197 (2007).', img: '/images/bookchap/1.png' }
            ], true)}
          </div>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="space-y-32">
            <section id="year-2020" className="scroll-mt-40">
              ...
              {renderCoverImages(['https://i.postimg.cc/J0j9GSnL/2020-1.png'])}
              {renderPubList([
                ...
                { num: 2, text: 'H. Zhao and Y. Lee et al., "Nanofabrication approaches for functional three-dimensional architectures", Nano Today, 30, 100825 (2020)', img: '/images/2020/2.jfif' },
            ...
            <section id="year-2019" className="scroll-mt-40">
              ...
              {renderCoverImages([
                'https://i.postimg.cc/d0Dp8h1B/2019-1.jpg',
                'https://i.postimg.cc/cJ62QvHD/2019-2.jpg'
              ])}
              {renderPubList([
                ...
                { num: 11, text: 'J.-B. Lee et al., "Direct Synthesis of Self-Assembled WSe2/MoS2 Heterostructure Array and Its Optoelectrical Properties", Advanced Materials 31, 1904194 (2019)', img: '/images/2019/11.jfif' },
                { num: 10, text: 'M. Shi, et al., "Boosting Ion Dynamics Through Superwettable Leaf-like Porous g-C3N4 Film for Ionogel Supercapacitors", NPG Asia Materials 11:61 (2019)', img: '/images/2019/10.jfif' },
                ...
                { num: 6, text: 'T. Nam et al., "Low-temperature, high-growth-rate ALD of SiO2 using aminodisilane precursor" Applied Surface Science 485, 381 (2019).', img: '/images/2019/6.jfif' },
                ...
                { num: 3, text: 'Y. J. Park, et al., "Controllable P- and N-type conversion of MoTe2 via oxide interfacial layer for logic circuits" Small, 15, 1901772 (2019).', img: '/images/2019/3-1.png' },
                ...
                { num: 1, text: 'Y. J. Park, et al., "All MoS2 Based Large Area, Skin-Attachable Active-Matrix Tactile Sensor" ACS Nano, 13, 3023 (2019).', img: '/images/2019/1.jfif' }
              ])}
            </section>
            ...
            <section id="year-2018" className="scroll-mt-40">
              ...
              {renderCoverImages([
                'https://i.postimg.cc/VN3Gn4pT/2018-1.jpg',
                'https://i.postimg.cc/Wz0Qhx40/2018-2.jpg'
              ])}
              {renderPubList([
                ...
                { num: 11, text: 'X. Chen, et al., "Degradation behaviors and mechanisms of MoS2 crystals relevant to bioabsorbable electronics", NPG Asia Materials, 10, 810 (2018)', img: '/images/2018/11.jfif' },
                ...
                { num: 5, text: 'M. Choi et al., "Flexible Active-Matrix Organic Light-Emitting Diode Display Enabled by MoS2 Thin-Film Transistor", Science Advances, 4, eaas8721 (2018)', img: '/images/2018/5.jfif' },
            ...
            <section id="year-2017" className="scroll-mt-40">
              ...
              {renderCoverImages(['https://i.postimg.cc/qRymLydq/2017-1.jpg'])}
              {renderPubList([
                ...
                { num: 1, text: 'B. Jang et al., "Damage mitigation in roll-to-roll transfer of CVD-graphene to flexible substrates", 2D Materials, 4 (2017)', img: '/images/2017/1.jfif' }
              ])}
            </section>
            ...
            <section id="year-2016" className="scroll-mt-40">
              ...
              {renderCoverImages([
                'https://i.postimg.cc/X73PsHqv/2016-1.jpg',
                'https://i.postimg.cc/g0fS841F/2016-2.png',
                'https://i.postimg.cc/50LP3LM1/2016-3.png',
                'https://i.postimg.cc/nckPg499/2016-4.jpg',
                'https://i.postimg.cc/QxbywQKF/2016-5.gif'
              ])}
              {renderPubList([
                ...
                { num: 9, text: 'H. Kim et al., "On-Fabrication Solid-State N-Doping of Graphene by an Polishing Metal Oxide Layer for Efficient Inverted Organic Solar Cells", Adv. Energy. Mat., 6, 1600172 (2016)', img: '/images/2016/9.png' },
                ...
                { num: 7, text: 'L. M. Loong et al., "Flexible MgO barrier magnetic tunnel junctions", Adv. Mat., 28, 4983-4990 (2016)', img: '/images/2016/7.jfif' },
                { num: 6, text: 'W. Lee et al., "Mobility enhancement of strained Si transistors by transfer printing on plastic substrates", NPG Asia Mat., 8, e256 (2016)', img: '/images/2016/6.jfif' },
                { num: 5, text: 'M. Park et al., "MoS2 based tactile sensor for electronic skin applications", Adv. Mat., 28, 2556 (2016)', img: '/images/2016/5-1.jpg' },
            ...
            <section id="year-2015" className="scroll-mt-40">
              ...
              {renderCoverImages([
                'https://i.postimg.cc/brf5cLR3/2015-1.jpg',
                'https://i.postimg.cc/QCZzGSq4/2015-2.jpg',
                'https://i.postimg.cc/fy7rwX2L/2015-3.jpg',
                'https://i.postimg.cc/ry5bCX8S/2015-4.jpg',
                'https://i.postimg.cc/jqy9Q0sM/2015-5.jpg',
                'https://i.postimg.cc/8PDYZKcV/2015-6.jpg'
              ])}
              {renderPubList([
                { num: 21, text: 'H. K. Seo et al., "Value-added Synthesis of Graphene: Recycling Industrial Carbon Waste into Electrodes for High Performance Electronic Devices", Scientific Report, 5, 16710 (2015)', img: '/images/2015/21.jfif' },
            ...
            <section id="year-2013" className="scroll-mt-40">
              ...
              {renderPubList([
                { num: 18, text: 'N. Gruhler et al., "High-quality Si3N4 circuits as a platform for graphene-based nanophotonic devices",Optics Express, 21, 31678, (2013).', img: '/images/2013/18.png' },
                { num: 17, text: 'J. Son et al., "Conductance modulation in topological insulator Bi2Se3 thin films with ionic liquid gating",Applied Physics Letters, 103, 213114 (2013)', img: '/images/2013/17.png' },
                { num: 16, text: 'C.A. Zaugg et al., "Ultrafast and widely tuneable vertical-external-cavity surface-emitting laser, mode-locked by a graphene-integrated distributed Bragg reflector", Optics Express, 21(25), 31548, (2013).', img: '/images/2013/16.png' },
                { num: 15, text: 'A. Pachoud et al., "Multiple Virtual Tunneling of Dirac Fermions in Granular Graphene", Scientific Reports, 3, 3404 (2013)', img: '/images/2013/15.png' },
                { num: 14, text: 'S.K. Lee et al., "Graphene based Transparent Conductive Film: Status and Perspective", Journal of the Korean Ceramic Society, 50(5), 309-318 (2013).', img: '/images/2013/14.png' },
                { num: 13, text: 'W. Lee et al., "Flexible graphene-PZT ferroelectric nonvolatile memory", Nanotechnology, 24, 475202 (2013)', img: '/images/2013/13.png' },
                { num: 12, text: 'H. Jang et al., "Quantum Confinement Effects in Transferrable Silicon Nanomembranes and Their Applications on Unusual Substrates", Nano Lett., 13, 5600-5607 (2013)', img: '/images/2013/12.png' },
                { num: 11, text: 'Yang Wu et al., "Graphene/liquid crystal based terahertz phase shifters", Optics Express, 21(18), 21402 (2013)', img: '/images/2013/11.png' },
                { num: 10, text: 'H.Y Jang et al., "Fabrication of Metallic Nanomesh: Pt Nano-Mesh as a Proof of Concept for Stretchable and Transparent Electrodes", Chem. Mater., 25, 3535-3538 (2013)', img: '/images/2013/10.png' },
                { num: 9, text: 'B.H. Sharma et al., "Graphene Based Field Effect Transistors: Efforts Made towards Flexible Electronics",Solid State Electronics, 89, 177-188 (2013)', img: '/images/2013/9.png' },
                { num: 8, text: 'Y. Lee et al., "Graphene-based transparent conductive films", Nano: Brief reports and review, 8(3), 1330001 (2013)', img: '/images/2013/8.png' },
                { num: 7, text: 'J. Kwon, et al., "Graphene Based Nanogenerator for Energy Harvesting",Japanese Journal of Applied Physics, 52 (2013)', img: '/images/2013/7.png' },
                { num: 6, text: 'S.H. Bae, et al., "Graphene-P(VDF-TrFE) multilayer film for flexible applications", ACS Nano, 7(4), 3130-3138 (2013)', img: '/images/2013/6.png' },
                { num: 5, text: 'J.E. Lee, et al., "Thermal Stability of Metal Ohmic Contacts in indium-gallium-zinc-oxide Transistors Using a Graphene Barrier Layer", Applied Physics Letters, 102, 113112 (2013)', img: '/images/2013/5.png' },
                { num: 4, text: 'S.K. Lee, et al., "Graphene Films for Flexible Organic and Energy Storage Devices", Journal of Physical Chemistry Letters, 4(5), 831-841 (2013)', img: '/images/2013/4.png' },
                { num: 3, text: 'A. A. Lagatsky, et al., "2 um solid-state laser mode-locked by single-layer graphene", Applied Physics Letters, 102, 013113 (2013)', img: '/images/2013/3.png' },
                { num: 2, text: 'Bhupendra K. Sharma, et al., "Load-Controlled Roll Transfer of Oxide Transistor for Stretchable Electronics", Adv. Func. Mat., 23, 2024-2032 (2013)', img: '/images/2013/2.png' },
                { num: 1, text: 'S.H. Bae, et al., "Graphene-based transparent strain sensor", Carbon, 51, 236 (2013)', img: '/images/2013/1.png' }
              ])}
            </section>
            ...
            <section id="year-2012" className="scroll-mt-40">
              ...
              {renderPubList([
                { num: 20, text: 'K. T. Eun, et al., "Mechanical flexibility of zinc oxide thin-film transistors prepared by transfer printing method", Modern Physics Letters B, 26, 1250077 (2012)', img: '/images/2012/20.png' },
                { num: 19, text: 'A. K. Patra, et al., "Dynamic spin injection into chemical vapor deposited graphene", Appl. Phys. Lett., 101, 162407 (2012)', img: '/images/2012/19.png' },
                { num: 18, text: 'Jing Niu, et al., "Shifting of surface plasmon resonance due to electromagnetic coupling between graphene and Au nanoparticles", Optics Express, 20, 18, 19690-19696 (2012)', img: '/images/2012/18.png' },
                { num: 17, text: 'Beom Joon Kim, et al., "Coplanar-Gate Transparent Graphene Transistors and Inverters on Plastic", ACS Nano, 6, 8646-8651 (2012)', img: '/images/2012/17.png' },
                { num: 16, text: 'Junggou Kwon, et al., "A High Performance PZT Ribbon-Based Nanogenerator using Graphene Transparent Electrodes", Energy & Environmental Science, 5, 8970-8975 (2012)', img: '/images/2012/16.png' },
                { num: 15, text: 'Young In Jhon, et al., "The mechanical responses of tilted and non-tilted grain boundaries in graphene",Carbon, 50, 3708-3716 (2012)', img: '/images/2012/15.png' },
                { num: 14, text: 'S.K. Lee, et al., "All Graphene based Thin Film Transistors on Flexible Plastic Substrates", Nano Lett., 12(7), 3472–3476 (2012)', img: '/images/2012/14.png' },
                { num: 13, text: 'Chao Yan, et al., "Graphene-Based Flexible and Stretchable Thin Film Transistors", Nanoscale, 4, 4870–4882 (2012)', img: '/images/2012/13.png' },
                { num: 12, text: 'Yong-Jin Kim, et al., "Low-temperature growth and direct transfer of graphene-graphitic carbon films on flexible plastic substrates", Nanotechnology, 23, 344016 (2012)', img: '/images/2012/12.png' },
                { num: 11, text: 'Byung-Jae Kim, et al., "Numerical Design of SiO2 Bridges in Stretchable Thin Film Transistors" Japanese Journal of Applied Physics, 51, 01AG10 (2012)', img: '/images/2012/11.png' },
                { num: 10, text: 'Dolly Shin, et al., "Synthesis and applications of graphene electrodes" Carbon Letters, 13, 1, 1-16, (2012)', img: '/images/2012/10.png' },
                { num: 9, text: 'Ji Tae Kim, et al., "Three-Dimensional Writing of Highly Stretchable Organic Nanowires", ACS Macro Lett., 1(3), 375–379 (2012)', img: '/images/2012/9.png' },
                { num: 8, text: 'Sukang Bae, et al., "Towards industrial applications of graphene electrodes" Physica Scripta T146, 014024 (2012)', img: '/images/2012/8.png' },
                { num: 7, text: 'Jing Niu, et al., "Graphene induced tunability of the surface plasmon resonance" Appl. Phys. Lett., 100, 061116, (2012).', img: '/images/2012/7.png' },
                { num: 6, text: 'Guang-Xin Ni, et al., "Quasi-Periodic Nanoripples in Graphene Grown by Chemical Vapor Deposition and Its Impact on Charge Transport", ACS Nano, 6(2), 1158–1164 (2012)', img: '/images/2012/6.png' },
                { num: 5, text: 'Jong-Hyun Ahn, et al., "Stretchable electronics: materials, architectures and integrations " J. of Physics D: Appl. Physics, 45, 103001, (2012)', img: '/images/2012/5.png' },
                { num: 4, text: 'Chao Yan, et al., "Mechanical and Environmental Stability of Polymer Thin-Film-Coated Graphene", ACS Nano, 6(3), 2096–2103 (2012)', img: '/images/2012/4.png' },
                { num: 3, text: 'T.H. Han, et al., "Extremely Efficient Flexible Organic Light-emitting Diodes with Modified Graphene Anode" Nature Photonics, 6, 105–110, (2012)', img: '/images/2012/3.png' },
                { num: 2, text: 'W.S. Lim, et al., "Atomic layer etching of graphene for full graphene device fabrication" Carbon, 50, 429-435 (2012)', img: '/images/2012/2.png' },
                { num: 1, text: 'Kun Seok Lee, et al., "Flexible and Platinum-Free Dye-Sensitized Solar Cells with Conducting-Polymer-Coated Graphene Counter Electrodes", Chemsuschem, 5, 379-382 (2012)', img: '/images/2012/1.png' }
              ])}
            </section>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="space-y-32">
            <section id="year-2026" className="scroll-mt-40">
              ...
              {renderPubList([
                { num: 10, text: 'P. Liu et al., "Multi-scaled Dynamic Interactions and Covalent Integration Enable Durable and Self-Healing Iontronic Skin for Wearable and Implantable Sensing", Adv. Func. Mate., e76648 (2026)', img: '/images/2026/10.jpg' },
                { num: 9, text: 'W. Guo, C. Wu, J. Choi, et al., "Layered Graphene/Hydrogel-based Multi-Modal Sensors Enabled by Ion-Electron Synergistic Conduction", Small, 22(40), e73949 (2026)', img: '/images/2026/9.jpg' },
            ...
            <section id="year-2025" className="scroll-mt-40">
              ...
              {renderCoverImages([
                'https://i.postimg.cc/fWX42Sv2/2025-1.png',
                'https://i.postimg.cc/zfMMpqbQ/2025-2.jpg',
                'https://i.postimg.cc/6pMF2rBk/2025-3.jpg',
                'https://i.postimg.cc/DZXNMR14/2025-4.jpg',
            ...
            <section id="year-2023" className="scroll-mt-40">
              ...
              {renderPubList([
                ...
                { num: 4, text: 'Y. Luo. et al., "Technology roadmap for flexible sensors", ACS Nano, 17, 5211 (2023)', img: '/images/2023/4.jfif' },
            ...
            <section id="year-2022" className="scroll-mt-40">
              ...
              {renderPubList([
                { num: 18, text: 'H. Kim et al., "Graphene nanopattern as a universal epitaxy platform for single-crystal membrane production and defect reduction", Nature Nanotechnology, 17, 1054 (2022)', img: '/images/2022/18.png' },
                { num: 17, text: 'Hu, L, Choi, J., Hwangbo, S et al., "Flexible micro-LED display and its application in Gbps multi-channel visible light communication", npj Flex Electron, 6, 100 (2022)', img: '/images/2022/17.png' },
                { num: 16, text: 'A. T. Hoang, L. Hu, A. Katiyar​ et al,. "Two-dimensional layered materials and heterostructures for flexible electronics", Matter, 5(12), 4116, (2022)​', img: '/images/2022/16.jfif' },
                { num: 15, text: 'L. Hu et al,. "Smart electronics based on 2D materials for wireless healthcare monitoring", Applied Physics Review, 9, 041308 (2022)​', img: '/images/2022/15.png' },
                { num: 14, text: 'Z. Chen et al., "Topography dependence of conductivity in electrostrictive germanium sulfide nanoribbons​", 2D Materials, 9, 045008 (2022)​', img: '/images/2022/14.jfif' },
                { num: 13, text: '​C. Lee et al., "A Miniaturized Wireless Neural Implant With Body-Coupled Power Delivery and Data Transmission", IEEE Journal of Solide-State Circuits (2022)​', img: '/images/2022/13.jfif' },
                { num: 12, text: 'Y. Liu et al., "Rational design of bioinspired gradient conductivity and stiffness for tactile sensors with high sensitivity and large linear range", Composites Science and Technology​, 228, 109674 (2022)', img: '/images/2022/12.jfif' },
                { num: 11, text: 'K. Kim et al., "In Situ Transmission Electron Microscopy Observation of 3C-SiC Heteroepitaxial Growth on Si Nanomembrane", Microscopy and Microanalysis 28 (S1), 1832 (2022)', img: '/images/2022/11.jfif' },
                { num: 10, text: 'L. P. L. Mawlong et al., “3D-Structured Photodetectors Based on 2D Transition-Metal Dichalcogenide”, Small Structures, 3, 2100149​ (2022)', img: '/images/2022/10.jpg' },
                { num: 9, text: 'L. Liu et al., "Biomimetic metal–organic framework-derived porous carbon welded carbon nanotube networks for strain sensors with high sensitivity and wide sensing range", Applied Surface Science, 593, 153417​ (2022)​', img: '/images/2022/9.png' },
                { num: 8, text: 'Z. Chen et al., "In-plane optical and electrical anisotropy in low-symmetry layered GeS microribbon", NPG Asia Mater., 14, 41 (2022)', img: '/images/2022/8.png' },
                { num: 7, text: 'S. Zhao and J.-H. Ahn, "Rational design of high-performance wearable tactile sensors utilizing bioinspired structures/functions, natural biopolymers, and biomimetic strategies", Materials Science & Engineering R, 148, 100672 (2022) (invited review article)​', img: '/images/2022/7.png' },
                { num: 6, text: 'M. Kang et al., "Wireless graphene-based thermal patch for obtaining temperature distribution and performing thermography​", Science Advances​ 8, eabm6693 (2022)​', img: '/images/2022/6.jfif' },
                { num: 5, text: 'K. Kim et al., "Observation of the Initial Stage of 3C-SiC Heteroepitaxial Growth on the Si Nanomembrane", Cryst. Growth Des.​, 22, 1421​ (2022)​', img: '/images/2022/5.png' },
                { num: 4, text: 'Y. Lee et al., "Morphable 3D Structure for Stretchable Display", Materials Today, 53, 51 (2022)', img: '/images/2022/4.jfif' },
                { num: 3, text: 'F. Liao et al., ​"Bioinspired in-sensor visual adaptation for accurate perception", Nature Electronics, 5, 84 (2022)', img: '/images/2022/3.png' },
                { num: 2, text: 'S. Hwangbo, L. Hu et al., "Wafer-scale monolithic integration of full-colour micro-LED display using MoS2 transistor", Nature Nanotechnology, 17, 500 (2022)​', img: '/images/2022/2.png' },
            ...
            <section id="year-2021" className="scroll-mt-40">
              ...
              {renderPubList([
                { num: 10, text: 'M. Lee et al., "Graphene-electrode array for brain-map remodeling of cortical surface​", NPG Asia Materials​, 13:65 (2021)', img: '/images/2021/10.jfif' },
                { num: 9, text: 'K. Thai et al., "MoS2/Graphene Photodetector Array with Strain-Modulated Photoresponse up to the Near-Infrared Regime", ACS Nano, 15, 12836 (2021)​', img: '/images/2021/9.png' },
                { num: 8, text: 'H. Kim et al., "Impact of 2D-3D heterointerface on Remote Epitaxial Interaction through Graphene", ACS Nano, 15, 10587​ (2021)', img: '/images/2021/8.png' },
                { num: 7, text: 'C. Kim et al., "Damage-free transfer mechanics of 2-dimensional materials: competition between adhesion instability and tensile strain", NPG Asia Materials,13, 44 (2021)', img: '/images/2021/7.png' },
                { num: 6, text: 'D.G. Roe et al., "Biologically Plausible Artificial Synaptic Array: Replicating Ebbinghaus’ Memory Curve with Selective Attention", Advanced Materials, 14, 2007782, (2021)', img: '/images/2021/6.png' },
                { num: 5, text: 'A Sarkar et al., "Si nanomebranes: Material properties and application" Nano Research, 14, 3010 (2021).', img: '/images/2021/5.png' },
                { num: 4, text: '​S.D. Kim et al., "Graphene-based Nanomaterials for Flexible and Stretchable Batteries", Small, 2006262​ (2021)​', img: '/images/2021/4.png' },
                { num: 3, text: 'A. T. Hoang et al., "Large-area synthesis of transition metal dichalcogenides via CVD and solution-based approaches and their device applications", Nanoscale, 13, 615 (2021)​', img: '/images/2021/3.png' },
                { num: 2, text: 'S. Zhao et al., "Polypyrrole-Coated Copper Nanowire-Threaded Silver Nanoflowers for Wearable Strain Sensors with High Sensing Performance", Chemical Engineering Journal, 417, 127966, (2021)​', img: '/images/2021/2.png' },
                { num: 1, text: '​J. Kim et al., "Two-Dimensional Materials for Skin Mountable Electronic Devices", Advanced Materials, 005858 (2021)', img: '/images/2021/1.jfif' }
              ])}
            </section>
          </div>
          <div className="mt-32 pt-16 border-t border-slate-200 flex justify-end"><button onClick={onBack} className="px-10 py-4 bg-[#131C31] text-white rounded-xl hover:bg-blue-600 transition-all font-bold shadow-lg flex items-center space-x-3 group"><span>Return to Lab Overview</span><ArrowLeft size={18} className="rotate-180 transform group-hover:translate-x-1 transition-transform" /></button></div>
  ...
  if (period === '2010') {
    const allPastPubs: PublicationItem[] = [
      // 2010
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] pb-20 relative">
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="pt-8">
            {renderPubList(allPastPubs, false, true)}
          </div>
          <div className="mt-32 pt-16 border-t border-slate-200 flex justify-end"><button onClick={onBack} className="px-10 py-4 bg-[#131C31] text-white rounded-xl hover:bg-blue-600 transition-all font-bold shadow-lg flex items-center space-x-3 group"><span>Return to Lab Overview</span><ArrowLeft size={18} className="rotate-180 transform group-hover:translate-x-1 transition-transform" /></button></div>
        </div>
      </div>
